...
def local_handler(data):
    print "Received data"
    data, hmacr = data[:-40], data[-40:]
    digest_maker = hmac.new('secret', '', hashlib.sha1)
    digest_maker.update(data)
    print "received hmac, computed hmac"
    print hmacr, digest_maker.hexdigest()	
    if hmacr == digest_maker.hexdigest():
        return data
    else:
        return ''
...
def remote_handler(data):
    digest_maker = hmac.new('secret', '', hashlib.sha1)
    digest_maker.update(data)
    print "Adding hmac", digest_maker.hexdigest()
    data = data + digest_maker.hexdigest()
    return data
...
LOCAL_DATA_HANDLER =   local_handler #lambda x:x
REMOTE_DATA_HANDLER =  remote_handler #lambda x:x
